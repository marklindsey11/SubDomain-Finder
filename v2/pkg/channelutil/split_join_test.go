...
func TestSplitCounter(t *testing.T) {
	...
	for n := 1; n < 101; n++ {
		...
		schan := channelutil.SplitChannels[struct{}]{
			MaxDrain:  3,
			Threshold: 5,
		}
		err := schan.Split(context.TODO(), source, sinks...)
		if err != nil {
			t.Fatalf(err.Error())
		}
	}
...
func TestSplitN(t *testing.T) {
	for n := 2; n < 44; n++ {
		...
		err := schan.Split(context.TODO(), source, recOnly...)
		if err != nil {
			t.Error(err)
		}
...
func TestNIntegration(t *testing.T) {
	// Integration Test i.e split and join
	...
	err := schan.Split(context.TODO(), domainChan, srcs...)
	if err != nil {
		t.Error(err)
	}
